...
class MDADConv(MessagePassing):
    def __init__(self, in_channels: int, out_channels: int,
        ...
        self.edge_attr_emb = Linear(edge_dim, edge_attr_emb_dim, bias=True)
        self.att = Linear(
            2 * out_channels + edge_type_emb_dim + edge_attr_emb_dim,
            self.heads, bias=False)
        self.lin = Linear(out_channels + edge_attr_emb_dim+edge_type_emb_dim, out_channels,
                          bias=bias)
    ...
    def forward(self, x: Tensor, edge_index: Adj, node_type: Tensor,
        ...
        edge_type_rep = F.leaky_relu(self.edge_type_rep(edge_type),
                                     self.negative_slope)
        out = self.propagate(edge_index, x=x, edge_type=edge_type_rep,
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_type: Tensor,
        ...
        edge_attr = F.leaky_relu(self.edge_attr_emb(edge_attr),
                                 self.negative_slope)
        edge_type = F.leaky_relu(self.edge_type_emb(edge_type),
                                 self.negative_slope)
        alpha = torch.cat([x_i, x_j, edge_type, edge_attr], dim=-1)
        alpha = F.leaky_relu(self.att(alpha), self.negative_slope)
        alpha = softmax(alpha, index, ptr, size_i)
        alpha = F.dropout(alpha, p=self.dropout, training=self.training)
        out = self.lin(torch.cat([x_j,edge_type, edge_attr], dim=-1)).unsqueeze(-2)
