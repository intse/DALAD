...
class TTDataset(Dataset):
    ...
    def raw_file_names(self):
        ...
        graphfile5 = "F5_trace_graph_depth10.txt"
        graphfile6 = "F6_trace_graph_depth10.txt"
        ...
        logfile5 = "F5_log_template_depth10.txt"
        logfile6 = "F6_log_template_depth10.txt"
        logfilebig = "Normalbig_log_template_depth10.txt"

        logvariablefile1="F1_log_variable_template_depth10_2processed.txt"
        logvariablefile2 = "F2_log_variable_template_depth10_2processed.txt"
        logvariablefile3 = "F3_log_variable_template_depth10_2processed.txt"
        logvariablefile5 = "F5_log_variable_template_depth10_2processed.txt"
        logvariablefile6 = "F6_log_variable_template_depth10_2processed.txt"
        logvariablefilebig = "Normalbig_log_variable_template_depth10_2processed.txt"

        F1_service_metricfile="F1_service.txt"
        F1_service_relation_metricfile="F1_relation_service.txt"

        F2_service_metricfile="F2_service.txt"
        F2_service_relation_metricfile="F2_relation_service.txt"

        F3_service_metricfile="F3_service.txt"
        F3_service_relation_metricfile="F3_relation_service.txt"

        F5_service_metricfile="F5_service.txt"
        F5_service_relation_metricfile="F5_relation_service.txt"

        F6_service_metricfile="F6_service.txt"
        F6_service_relation_metricfile="F6_relation_service.txt"

        Normalbig_service_metricfile="Normalbig_service.txt"
        Normalbig_service_relation_metricfile="Normalbig_relation_service.txt"
        ...
        return [graphfile1,graphfile2,graphfile3,graphfile5,graphfile6,graphfilebig,
                logfile1,logfile2,logfile3,logfile5,logfile6,logfilebig,
                logvariablefile1,logvariablefile2,logvariablefile3,logvariablefile5,logvariablefile6,logvariablefilebig,
                ...
                F5_service_metricfile, F5_service_relation_metricfile,
                F6_service_metricfile, F6_service_relation_metricfile,
                Normalbig_service_metricfile, Normalbig_service_relation_metricfile,
                vectortemplatefile,logvariableVectorTemplateFile,SpanTemplateIdfile]
    ...
    def process(self):
        All_trace = set()
        with open("./OBD/TT/raw/trace_OCV.txt", "r", encoding='utf-8') as f:
            lines = f.readlines()
            for l in lines:
                l = l.replace("\n", "").strip().split(":")
                traceid = l[0]
        ...
        SpanTemplateId=set()
        with open(self.raw_paths[-1], encoding='utf-8-sig') as f:
            for line in f:
                line = line.replace("\n", "").strip()
                SpanTemplateId.add(line)
        f.close()

        with open(self.raw_paths[-2], encoding='utf-8-sig') as f:
            for line in f:
                line = line.replace("\n", "").strip().split()
                id = line[0]
        ...
        with open(self.raw_paths[-3], encoding='utf-8-sig') as f:
            for line in f:
                line = line.replace("\n", "").strip().split()
                id = line[0]
        ...
        for i in range(5):
            ...
            with open(self.raw_paths[targetIndex],"r",encoding='utf-8') as f:
                lines=f.readlines()
                for line in lines:
                    line = line.replace("\n","").strip().split()
                    service_name=line[0]
        ...
        logfilelist=self.raw_paths[6:12]
        logVector={}
        logType={}
        logServiceName={}
        logTimeStamp={}
        ...
        variablefilelist=self.raw_paths[12:18]
        variabelVector={}
        for i in range(len(variablefilelist)):
            with open(variablefilelist[i], encoding='utf-8') as f:
                for line in f:
                    line = line.strip().replace("\n", "").split()
        ...
        graphfilelist=self.raw_paths[:6]
        idx=0
        for i in range(len(graphfilelist)):
            with open(graphfilelist[i], 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if i==len(graphfilelist)-1:
                        ...
                        NF_label = 1
                    labels.append([NF_label, i, label])
                    ...
                    data = Data(x=X,variableX=variableX, ServiceX=SerX,node_type=NodeType,edge_type=EdgeType,edge_index=edge_index, edge_attr=EdgeAttr, y=y)
